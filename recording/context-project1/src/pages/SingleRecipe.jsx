...
const SingleRecipe = () => {
  ...
  useEffect(() => {
    ...
    setIsFav(favs.some((item) => item.id === recipe.id));
  }, [recipe]);
  ...
  const updateHandler = (formData) => {
    ...
    const updatedFavs = favs.map((item) =>
      item.id === id ? { ...item, ...formData } : item
    );
  ...
  const favoriteHandler = () => {
    let favs =mJSON.parse(localStorage.getItem("favoriteRecipes")) || [];
    ...
    if (exists) {
      favs = favs.filter( (item) => item.id !== recipe.id);
        ...
        <div className="bg-zinc-900 rounded-3xl overflow-hidden shadow-xl">
          ...
          <div className="relative">
            ...
            <button
              ...
              className="absolute top-5 right-5 bg-white w-14 h-14 rounded-full flex justify-center items-center shadow-xl hover:scale-110 duration-300"
            >
              <i
                className={
                  isFav
                    ? "ri-heart-fill text-red-500 text-3xl"
                    : "ri-heart-line text-black text-3xl"
                }
              ></i>
            </button>
          ...
          <div className="p-8">
            ...
            <p className="leading-8 text-gray-300 mt-6">
